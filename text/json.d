...
abstract class JSONType {
	...
	// The parse method of this interface should ALWAYS be destructive, 
	// removing things from the front of source as it parses.
	...
	JSONType opIndex(string key) {
		throw new JSONTypeError(typeof(this).stringof ~" does not support string indexing, check your JSON structure.");
	}
	
	/// Allow foreach over the object with string key and ref value.
	int opApply(int delegate(ref string,ref JSONType) dg) {
		throw new JSONTypeError(typeof(this).stringof ~" does not support string index foreach, check your JSON structure.");
	}
	
	/// Array index function for objects describing array-like attributes.
	/// Returns: The chosen index or a null reference if the index does not exist.
	JSONType opIndex(int key) {
		throw new JSONTypeError(typeof(this).stringof ~" does not support integer indexing, check your JSON structure.");
	}
	
	/// Allow foreach over the object with integer key and ref value.
	int opApply(int delegate(ref ulong,ref JSONType) dg) {
		throw new JSONTypeError(typeof(this).stringof ~" does not support numeric index foreach, check your JSON structure.");
	}
	...
	int opApply(int delegate(ref JSONType) dg) {
		throw new JSONTypeError(typeof(this).stringof ~" does not support foreach, check your JSON structure.");
	}
	
	/// Allow "in" operator to work as expected for object types without an explicit cast
	JSONType*opIn_r(string key) {
		throw new JSONTypeError(typeof(this).stringof ~" does not support opIn, check your JSON structure.");
	}
}
/**
...
class JSONObject:JSONType {
	...
	override void parse(ref string source) {
		...
		if (source[0] != '{') throw new JSONException("Missing open brace '{' at start of JSONObject parse: "~source);
		// rip off the leading {
		source = stripLeft(source[1..$]);
		while (source[0] != '}') {
			if (source[0] != '"') throw new JSONException("Missing open quote for element key before: "~source);
			// use JSONString class to help us out here (read, I'm lazy :D)
			auto jstr = new JSONString();
			jstr.parse(source);
			source = stripLeft(source);
			if (source[0] != ':') throw new JSONException("Missing ':' after keystring in object before: "~source);
			source = stripLeft(source[1..$]);
			_children[jstr.get] = parseHelper(source);
			source = stripLeft(source);
			// handle end cases
			if (source[0] == '}') continue;
			if (source[0] != ',') throw new JSONException("Missing continuation via ',' or end of JSON object via '}' before "~source);
			// rip the , in preparation for the next loop
			source = stripLeft(source[1..$]);
			// make sure we don't have a ",}", since I'm assuming it's not allowed
			if (source[0] == '}') throw new JSONException("Empty array elements (',' followed by '}') are not allowed. Fill the space or remove the comma.\nThis error occurred before: "~source);
		}
...
class JSONArray:JSONType {
	...
	override void parse(ref string source) {
		if (source[0] != '[') throw new JSONException("Missing open brace '[' at start of JSONArray parse: "~source);
		// rip off the leading [
		source = stripLeft(source[1..$]);
		while (source[0] != ']') {
			...
			// handle end cases
			if (source[0] == ']') continue;
			if (source[0] != ',') throw new JSONException("Missing continuation via ',' or end of JSON array via ']' before "~source);
			// rip the , in preparation for the next loop
			source = stripLeft(source[1..$]);
...
class JSONString:JSONType {
	...
	override void parse(ref string source) {
		if (source[0] != '"') throw new JSONException("Missing open quote '\"' at start of JSONArray parse: "~source);
		// rip off the leading [
		source = source[1..$];
		...
		if (sliceloc == -1) throw new JSONException("Unable to find the end of the JSON string starting here: "~source);
		_data = source[0..sliceloc];
		// eat the " that is known to be there
		source = stripLeft(source[sliceloc+1..$]);
	}
}
...
class JSONBoolean:JSONType {
	...
	override void parse(ref string source) {
		if (source[0..4] == "true") {
			source = stripLeft(source[4..$]);
			set(true);
		} else if (source[0..5] == "false") {
			source = stripLeft(source[5..$]);
			set(false);
		} else throw new JSONException("Could not parse JSON boolean variable from: "~source);
	}
}
...
class JSONNumber:JSONType {
	...
	override void parse(ref string source) {
		...
		} else throw new JSONException("A numeric parse error occurred while parsing the numeric beginning at: "~source);
...
private JSONType parseHelper(ref string source) {
	...
	default:
		...
		else throw new JSONException("Unable to determine type of next element beginning: "~source);
		break;
	}
	ret.parse(source);
	return ret;
...
unittest {
	...
	} catch (Error e) {/*shazam! program flow should get here, it is a correct thing*/}
